...
import 'package:friday_sa/features/wallet/domain/models/wallet_filter_body_model.dart';
...
class WalletController extends GetxController implements GetxService {
  ...
  Future<void> addFundToWallet(num amount, String paymentMethod) async {
    _isLoading = true;
    update();
    Response response = await walletServiceInterface.addFundToWallet(
      amount,
      paymentMethod,
    );
  ...
  Future<void> getWalletBonusList({bool isUpdate = true}) async {
    ...
    List<FundBonusModel>? bonuses = await walletServiceInterface
        .getWalletBonusList();
    if (bonuses != null) {
      _fundBonusList = [];
