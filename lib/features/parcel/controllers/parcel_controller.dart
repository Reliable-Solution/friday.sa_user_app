...
import 'package:friday_sa/features/parcel/domain/models/place_details_model.dart';
...
class ParcelController extends GetxController implements GetxService {
  ...
  Future<void> getParcelCategoryList() async {
    List<ParcelCategoryModel>? categoryModelList = await parcelServiceInterface
        .getParcelCategory();
  ...
  Future<void> setLocationFromPlace(
    String? placeID,
    String? address,
    bool? isPickedUp,
  ) async {
  ...
  Future<void> _processAddressAndAction(
  ...
  Future<void> getVideoContentDetails({
    ...
    if (source == DataSourceEnum.local) {
      _videoContentDetails = await parcelServiceInterface
          .getVideoContentDetails(source: source);
      update();
  ...
  Future<void> getParcelInstruction() async {
    ...
    _parcelInstructionList = await parcelServiceInterface.getParcelInstruction(
      1,
    );
    update();
  }
