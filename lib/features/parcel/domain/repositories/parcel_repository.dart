...
import 'package:friday_sa/features/parcel/domain/models/parcel_instruction_model.dart';
...
class ParcelRepository implements ParcelRepositoryInterface {
  ...
  Future get(
    ...
    if (isVideoDetails) {
      return _getVideoContentDetails(source: source);
    } else {
      return _getWhyChooseDetails(source: source);
    }
  }
    ...
    switch (source) {
      ...
      case DataSourceEnum.local:
        ...
        if (cacheResponseData != null) {
          videoContentDetails = VideoContentModel.fromJson(
  ...
  Future<WhyChooseModel?> _getWhyChooseDetails({
    ...
    switch (source) {
      ...
      case DataSourceEnum.local:
        ...
        if (cacheResponseData != null) {
          whyChooseDetails = WhyChooseModel.fromJson(
            jsonDecode(cacheResponseData),
          );
        }
    }
  ...
  Future getList({int? offset, bool parcelCategory = true}) async {
    if (parcelCategory) {
      return _getParcelCategory();
    } else {
      return _getParcelInstruction(offset!);
    }
  }
  ...
  Future<List<Data>?> _getParcelInstruction(int offset) async {
    ...
    if (response.statusCode == 200) {
      ...
      parcelInstructionList.addAll(
        ParcelInstructionModel.fromJson(response.body).data!,
      );
    }
