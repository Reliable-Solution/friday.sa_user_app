...
import 'package:friday_sa/features/chat/domain/models/chat_model.dart';
...
class ChatController extends GetxController implements GetxService {
  ...
  Future<void> getConversationList(int offset, {String type = ''}) async {
    ...
    if (conversationModel != null) {
      ...
      int index0 = chatServiceInterface.setIndex(
        _conversationModel!.conversations,
      );
      bool sender = chatServiceInterface.checkSender(
        _conversationModel!.conversations,
      );
      _hasAdmin = false;
  ...
  Future<void> searchConversation(String name) async {
    _searchConversationModel = ConversationsModel(conversations: []);
    update();
    ...
    if (name.contains('admin') ||
      ...
      _searchConversationModel!.conversations!.add(
        Conversation(
          id: 0,
          senderId: 0,
  ...
  Future<void> getMessages(
    ...
    if (response != null &&
      ...
      if (offset == 1) {
        ...
        _messageModel!.messages!.addAll(
          ChatModel.fromJson(response.body).messages!,
        );
      }
    }
    update();
  ...
  Future<Response?> sendMessage({
    ...
    List<MultipartBody> myImages = chatServiceInterface.processMultipartBody(
      _chatImage,
    );
    ...
    if (response!.statusCode == 200) {
      ...
      if (Get.find<ProfileController>().userInfoModel!.userInfo == null) {
        Get.find<ProfileController>().updateUserWithNewData(
