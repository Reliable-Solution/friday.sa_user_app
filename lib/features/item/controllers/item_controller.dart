...
import 'package:friday_sa/features/item/domain/models/common_condition_model.dart';
...
class ItemController extends GetxController implements GetxService {
  ...
  Future<void> getProductDetails(Item item) async {
    ...
    setExistInCart(
      _item,
      _selectedVariations /*notify: !ResponsiveHelper.isDesktop(Get.context)*/,
    );
  }
  ...
  void itemDirectlyAddToCart(
    ...
    if (((item!.foodVariations != null && item.foodVariations!.isEmpty) &&
      ...
      num discountPrice = PriceConverter.convertWithDiscount(
        price,
        discount,
        item.discountType,
      )!;
      ...
      "===>>>> ModuleConfig.fromJson ${Get.find<SplashController>().configModel!.moduleConfig!.toJson()}"
          .print;
      if (Get.find<SplashController>()
              .configModel!
              .moduleConfig!
