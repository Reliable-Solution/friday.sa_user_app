  ...
  Future<void> getFavouriteList() async {
    ...
    if (response.statusCode == 200) {
          ...
          if (item['module_type'] == null ||
            ...
            if (Get.find<SplashController>().module == null) {
              ...
              _wishItemIdList.addAll(
                favouriteServiceInterface.wishItemIdList(i),
              );
            } else {
              _wishItemList!.add(i);
      ...
      response.body['store'].forEach((store) async {
        if (Get.find<SplashController>().module == null) {
          _wishStoreList!.addAll(
            favouriteServiceInterface.wishStoreList(store),
          );
          _wishStoreIdList.addAll(
            favouriteServiceInterface.wishStoreIdList(store),
          );
        } else {
          Store? s;
          try {
