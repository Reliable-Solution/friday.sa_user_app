...
import 'package:friday_sa/features/cart/widgets/extra_packaging_widget.dart';
import 'package:friday_sa/features/cart/widgets/not_available_bottom_sheet_widget.dart';
...
import 'package:friday_sa/helper/module_helper.dart';
...
import 'package:friday_sa/common/widgets/item_widget.dart';
import 'package:friday_sa/common/widgets/menu_drawer.dart';
import 'package:friday_sa/common/widgets/no_data_screen.dart';
import 'package:friday_sa/common/widgets/web_constrained_box.dart';
import 'package:friday_sa/common/widgets/web_page_title_widget.dart';
import 'package:friday_sa/features/cart/widgets/cart_item_widget.dart';
import 'package:flutter/material.dart';
import 'package:get/get.dart';
import 'package:friday_sa/features/cart/widgets/web_cart_items_widget.dart';
import 'package:friday_sa/features/cart/widgets/web_suggested_item_view_widget.dart';
            ...
            children: [
              ...
              !ResponsiveHelper.isDesktop(context) &&
                      splashController
                          ...
                          .newVariation! &&
                      (storeController.store != null &&
                          (storeController.store?.cutlery ?? false))
                  ? Padding(
                      padding: const EdgeInsets.symmetric(
                        horizontal: Dimensions.paddingSizeDefault,
                        vertical: Dimensions.paddingSizeSmall,
            ...
            builder: (storeController) {
              ...
              if (storeController.cartSuggestItemModel != null) {
                ...
                for (CartModel cartItem in cartList) {
                  cartIds.add(cartItem.item?.id ?? 0);
                }
                for (Item item
              ...
              return storeController.cartSuggestItemModel != null &&
                      (suggestedItems?.isNotEmpty ?? false)
                  ? Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const SizedBox(height: Dimensions.paddingSizeSmall),
                        ...
                        SizedBox(
                          ...
                          child: ListView.builder(
                            ...
                            itemCount: suggestedItems?.length ?? 0,
...
num _calculateOriginalDeliveryCharge({
  ...
  if (store != null &&
    ...
    perKmCharge = store.perKmShippingCharge?.printWithName(
      'store.perKmShippingCharge',
    );
    minimumCharge = store.minimumShippingCharge?.printWithName(
      'store.minimumShippingCharge',
    );
    maximumCharge = store.maximumShippingCharge.printWithName(
      'store.maximumShippingCharge',
    );
  } else if (store != null &&
      distance != null &&
      distance != -1 &&
      moduleData != null) {
    perKmCharge = moduleData.perKmShippingCharge?.printWithName(
      'moduleData.perKmShippingCharge',
    );
    minimumCharge = moduleData.minimumShippingCharge?.printWithName(
      'moduleData.minimumShippingCharge',
    );
    maximumCharge = moduleData.maximumShippingCharge.printWithName(
      'moduleData.maximumShippingCharge',
    );
  }
  if (store != null && distance != null) {
    deliveryCharge = distance * perKmCharge;
    print("===>>> here calculation 6 $distance $deliveryCharge");
    if (deliveryCharge.printWithName("deliveryCharge") <
        minimumCharge.printWithName("minimumCharge")) {
      deliveryCharge = minimumCharge;
    } else if (maximumCharge != null && deliveryCharge > maximumCharge) {
      deliveryCharge = maximumCharge;
    }
  }
  print("===>>> here calculation 7 $deliveryCharge");
  ...
  print("===>>> here calculation  $deliveryCharge");
...
class CheckoutButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    double? percentage;
    final splashController = Get.find<SplashController>();
    final distance = Get.find<CheckoutController>().distance;
    final extraCharge = Get.find<CheckoutController>().extraCharge;
    ...
    return Container(
      ...
      child: GetBuilder<StoreController>(
        builder: (storeController) {
          final adminFreeDelivery =
              splashController.configModel?.adminFreeDelivery;
          final storeController = Get.find<StoreController>();
          final originalDelivery = _calculateOriginalDeliveryCharge(
            store: storeController.store,
            address: AddressHelper.getUserAddressFromSharedPref()!,
            distance: distance,
            extraCharge: extraCharge,
          );
          if (!(storeController.store?.freeDelivery ?? false) &&
              ((adminFreeDelivery?.status ?? false) &&
                  (adminFreeDelivery?.type ==
                      'free_delivery_by_order_amount') &&
                  (adminFreeDelivery?.freeDeliveryOver != null))) {
            percentage =
                cartController.subTotal /
                (adminFreeDelivery?.freeDeliveryOver ?? 1);
          }
          final isPercentage =
              (percentage?.print ?? 0) <= 1 && originalDelivery > 0;
          return Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              (percentage != null)
                  ? Column(
                      children: [
                        Row(
                          mainAxisAlignment: isPercentage
                  ...
                  onPressed: () {
                    if (!(cartController.cartList.first.item?.scheduleOrder ??
                      ...
                      if (splashController.module == null) {
                        ...
                        for (
                          i = 0;
                          i < (splashController.moduleList?.length ?? 0);
                          i++
                        ) {
                          if (cartController.cartList[0].item?.moduleId ==
                              splashController.moduleList![i].id) {
                            break;
                          }
                        }
                        splashController.setModule(
                          splashController.moduleList![i],
                        );
