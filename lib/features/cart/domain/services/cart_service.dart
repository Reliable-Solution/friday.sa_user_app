...
import 'package:friday_sa/features/cart/domain/models/online_cart_model.dart';
...
class CartService implements CartServiceInterface {
  ...
  num calculateVariationWithoutDiscountPrice(
    ...
    num variationWithoutDiscountPrice = variationWithoutDiscount;
    if (!isFoodVariation) {
      String variationType = '';
      for (int i = 0; i < cartModel.variation!.length; i++) {
        variationType = cartModel.variation![i].type!;
  ...
  List<CartModel> formatOnlineCartToLocalCart({
    ...
    for (OnlineCartModel cart in onlineCartModel) {
      num price = cart.item!.price!;
      num? discount = cart.item!.storeDiscount == 0
          ? cart.item!.discount!
          : cart.item!.storeDiscount!;
      String? discountType = (cart.item!.storeDiscount == 0)
          ? cart.item!.discountType
          : 'percent';
      num discountedPrice = PriceConverter.convertWithDiscount(
        price,
        discount,
        discountType,
      )!;

      num? discountAmount = price - discountedPrice;
