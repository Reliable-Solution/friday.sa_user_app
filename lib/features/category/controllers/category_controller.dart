...
class CategoryController extends GetxController implements GetxService {
  ...
  List<CategoryModel> _storeCategoryList = [];
  List<CategoryModel> get storeCategoryList => _storeCategoryList;
  ...
  Future<List<CategoryModel>?> getStoreCategoryList(
    ...
    if (_storeCategoryList.isEmpty || reload || fromRecall) {
      ...
      if (dataSource == DataSourceEnum.local) {
        storeCategoryList = await categoryServiceInterface.getStoreCategoryList(
          storeId,
          source: DataSourceEnum.local,
        );
        ...
        "=====>>> storeCategoryList _store!.categoryIds ${storeCategoryList?.length}"
            .print;
        list = storeCategoryList;
        _prepareStoreCategoryList(storeCategoryList);
      }
  ...
  _prepareStoreCategoryList(List<CategoryModel>? storeCategoryList) {
    if (storeCategoryList != null) {
      for (var category in storeCategoryList) {
        if (!_storeCategoryList.any((element) => category.id == element.id)) {
          _storeCategoryList.add(category);
        }
      }
    }
    update();
