...
class CentralizeLoginHelper {
  static ({CentralizeLoginType type, double size}) getPreferredLoginMethod(
    CentralizeLoginSetup data,
    bool isOtpViewEnable, {
    bool calculateWidth = false,
  }) {
    if ((data.otpLoginStatus! &&
            !data.manualLoginStatus! &&
            !data.socialLoginStatus!) ||
        isOtpViewEnable) {
      return (type: CentralizeLoginType.otp, size: 400);
    } else if (data.manualLoginStatus! &&
        !data.socialLoginStatus! &&
        !data.otpLoginStatus!) {
      return (type: CentralizeLoginType.manual, size: 500);
    } else if (data.socialLoginStatus! &&
        !data.otpLoginStatus! &&
        !data.manualLoginStatus!) {
